...
#original wide data
path <- system.file("extdata",package = "dslabs")
filename <- file.path(path,"fertilty-two-countries-example.csv")
wide_data <- read_csv(filename)
...
# Gather all columns exccept country
new_tidy_data <- wide_data %>% gather(year,fertility,-country)

# Gather treats column names as  charactersby default
...
# split on first underscores but keep life_expectanncy merged
dat %>% separate(key,c("year","variable_name"),sep = "_",extra="merge") %>% spread(variable_name,value)

#separate then unite
dat %>% separate(key, c("year","first_variable_name","second_variable_name"),fill="right") %>% unite(variable_name,first_variable_name,second_variable_name,sep="_")
